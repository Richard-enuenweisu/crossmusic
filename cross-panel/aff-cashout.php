...
if (!isset($_SESSION['AFFILIATE_ID'])) {
  permission_error('login.php');
	...
	$affliliate_id = $_SESSION['AFFILIATE_ID'];
	$stmt = $pdo->query("SELECT * FROM aff_accounttbl WHERE id = $affliliate_id ");
	$acc_result = $stmt->fetch(PDO::FETCH_ASSOC);
	$lname = $acc_result['lname'];

}

  $bal_query = $pdo->query("SELECT * FROM aff_balancetbl WHERE account_id = $affliliate_id ");
...
if (isset($_POST['submit'])) {
  ...
  if (empty($errors)) {
	...
	// try {
            ...
            <table align="center" bgcolor="#fff" border="0" cellpadding="0" cellspacing="0" height="100%" width="100%">
                <tbody><tr>
                    <td align="center" valign="top" style="padding-bottom:60px">
                        <p style="color:#ffe01b;display:none;font-size:0px;height:0px;width:0px">Click to view new request!</p>
                                    ...
                                    <table align="center" bgcolor="#FFFFFF" border="0" cellpadding="0" cellspacing="0" style="background-color:#ffffff;max-width:640px" width="100%" class=">
                                        <tbody><tr>
                                            <td align="center" valign="top" bgcolor="#FFFFFF" style="padding-right:40px;padding-bottom:40px;padding-left:40px">
                                                <h1 style="color:#241c15;font-family:Georgia,Times,Times New Roman,serif;font-size:30px;font-style:normal;font-weight:400;line-height:42px;letter-spacing:normal;margin:0;padding:0;text-align:center">New Affiliate Cashout Request from,<br> '
                                                .ucfirst($lname).'</h1>
                                            </td>
                                        </tr>
                                        <tr>
	    ...
	    $stmt = $pdo->prepare("INSERT INTO `aff_transtbl` ( `account_id`, `description`, `amount`, `curr_bal`) VALUES(:account_id, :description, :amount, :curr_bal)");
	    $stmt->execute([':account_id'=>$affliliate_id, ':description'=>$description, ':amount'=>$amount, ':curr_bal'=>$curr_balance]);
			...
			<div class="row">
				<div class="col-lg-6 d-flex flex-column justify-content-center align-items-center bg-white mnh-100vh">
					...
					<div class="u-login-form">
						<form class="mb-3" method="POST" action="aff-cashout.php">
							<div class="mb-3">
