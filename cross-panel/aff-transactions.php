...
if (!isset($_SESSION['AFFILIATE_ID'])) {
  permission_error('login.php');
	...
	$affliate_id = $_SESSION['AFFILIATE_ID'];
	$stmt = $pdo->query("SELECT * FROM aff_accounttbl WHERE id = $affliate_id ");
	$acc_result = $stmt->fetch(PDO::FETCH_ASSOC);
}

$trans_query = $pdo->query("SELECT * FROM aff_transtbl WHERE account_id = $affliate_id ORDER BY id DESC");
