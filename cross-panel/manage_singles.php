  ...
  // Validate file input to check if is with valid extension
    ...
    if(empty($errors)) {
		...
		//Extract 30 seconds starting after 10 seconds.
            ...
            $insert_query = $pdo->prepare("INSERT INTO  singlestbl ( `acount_id`, `title`, `price`, `duration`, `short_url`, `long_url`, `image`) VALUES (:acount_id, :title, :price, :duration, :short_url, :long_url, :image)");
            $insert_query->execute([':acount_id' =>$artist_id, ':title' =>$title, ':price'=>$price, ':duration'=>$duration, ':short_url'=>$short_track_path, ':long_url'=>$track_path, ':image'=>$cover_path]);            
...
if (isset($_GET['edit'])) {
	...
	if (isset($_POST['edit'])) {
	    ...
	    if (isset($title) && !empty($title)) {
	      # code...
	      $stmt =$pdo->prepare("UPDATE `singlestbl` SET `title`=:title WHERE id =:edit_id");
	      $stmt->execute([':title'=>$title, ':edit_id'=>$single_id]);
	      $success ='Updated successfully.';
	      header('refresh:2; url = manage_singles.php?edit='.$single_id);
	    }
	    if (isset($price) && !empty($price) && $price <= 200) {
	      # code...
	      $stmt =$pdo->prepare("UPDATE `singlestbl` SET `price`=:price WHERE id =:edit_id");
	      $stmt->execute([':price'=>$price, ':edit_id'=>$single_id]);
	      $success =' Updated successfully.';
	      header('refresh:2; url = manage_singles.php?edit='.$single_id);
	    }   
	    if (isset($duration) && !empty($duration)) {
	      # code...
	      $stmt =$pdo->prepare("UPDATE `singlestbl` SET `duration`=:duration WHERE id =:edit_id");
	      $stmt->execute([':duration'=>$duration, ':edit_id'=>$single_id]);
	      $success = 'Updated successfully.';
	      header('refresh:2; url = manage_singles.php?edit='.$single_id);
	    }     
	    if (isset($track) && !empty($track)) {
		  ...
		  // Validate file input to check if is with valid extension
			...
			//Extract 30 seconds starting after 10 seconds.
		      ...
		      $stmt =$pdo->prepare("UPDATE `singlestbl` SET `short_url`=:short_url,`long_url`=:long_url WHERE id =:edit_id");
		      $stmt->execute([':short_url'=>$short_track_path, ':long_url'=>$track_path, ':edit_id'=>$single_id]);
		      $success='Updated successfully.';
		      header('refresh:2; url = manage_singles.php?edit='.$single_id);
	        } else {
	    ...
	    if (isset($artcover) && !empty($artcover)) {
			...
			//update cover				      	
		      $stmt =$pdo->prepare("UPDATE `singlestbl` SET `image`=:image WHERE id =:edit_id");
		      $stmt->execute([':image'=>$cover_path, ':edit_id'=>$single_id]);
		      $success= 'Updated successfully.';
		      header('refresh:2; url = manage_singles.php?edit='.$single_id);
	        } else {
			...
			<div class="row">
				<div class="col-lg-6 d-flex flex-column justify-content-center align-items-center bg-white mnh-100vh">
					...
					<div class="u-login-form">
						<form class="mb-3" method="POST" action="manage_singles.php?<?=(isset($_GET['edit']))?'edit='.$single_id:'Add'?>" enctype="multipart/form-data">
							<div class="mb-3">
